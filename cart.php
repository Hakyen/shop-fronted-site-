					...
					<div class="col-md-12">
						...
						<div class="chart-item table-responsive fix">
							<table class="col-md-12">
								<thead>
									<tr>
										<th class="th-delate">Удалить</th>
										<th class="th-product">Фото</th>
										<th class="th-details">Назввние товара</th>
										<th class="th-qty">Колличество</th>
										<th class="th-total">Цена за 1</th>
										<th class="th-total">Итог</th>
									</tr>
								</thead>
								<tbody>
									<?php if( !empty( $_SESSION['products'] ) ) { foreach ( $_SESSION['products'] as $key => $item ) {
										$product = $dbh->query("SELECT * FROM products WHERE idProducts = '" . $key . "'")->fetch();
									?>
									<tr>
										<td class="th-delate"><a href="/diplom_shop/cart.php?id=<?=$product['idProducts'];?>">X</a></td>
										<td class="th-product">
											<a href="/diplom_shop/product-details.php?id=<?=$product['idProducts'];?>"><img src="<?=$product['pictures'];?>" alt="cart"></a>
										</td>
										<td class="th-details">
											<h2><a href="/diplom_shop/product-details.php?id=<?=$product['idProducts'];?>"><?=$product['name'];?></a></h2>
										</td>
										<td class="th-qty"><?=$item;?></td>
										<td class="th-total"><?=$product['price'];?>р</td>
										<td class="th-total"><?=$product['price'] * $item;?></td>
									</tr>
								<?php } } ?>
								</tbody>
							</table>
